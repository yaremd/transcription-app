...
The installer window has a second script, because it is a layout rather than a
mark — a canvas with the lockup, the two icon positions and the arrow between
them. Its constants and the ones in `scripts/make-dmg.sh` are one design in two
...
| `out/dmg/*` | generated by `gen-dmg-background.swift` · the disk image window art. `background.tiff` is the 1x and 2x pair welded together — Finder takes one file and picks the representation, and the plain PNG ships visibly soft. `scripts/make-dmg.sh` builds it into the DMG. |
